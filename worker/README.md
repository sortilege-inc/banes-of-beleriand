...
`engine/ops.js` (and `system/tor2e/ops.js`) the browser uses, validates them by role (the GM
...
```bash
cd worker && npm install && npx wrangler dev --port 8793   # local; the app on localhost talks to it
