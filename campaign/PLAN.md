...
**O2 — open: Behind the Veil (asked 2026-09-24, U3).** PLAYBOOK §4b puts the GM's material in the
GM tabs (Overview, Scenes, Threads, People, Places, Notes), in the pack; this instance still has
M2's *Behind the Veil* panel (`campaign/site/gm.js`): `campaign/docs/veil/*.md` behind its own
spoiler gate, plus the Company loader. The veil folder is empty (`build_docs: … veil 0`). Retire
the veil for the GM tabs now and keep only the loader, or keep it until there is content? Nothing
is changed until the owner answers.
...
**O2** (above) — Behind the Veil, the owner's call.
