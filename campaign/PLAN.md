...
| 25 | Lily's Virtue *Hardiness* is referenced by the core's entity `#tG8lehN34BzH8fdIUfnW3PK`, which the corpus does **not** type as a Virtue | In `tor2e-0.5-core-valour-and-wisdom.ttrpg` Hardiness sits outside the *Virtues List* with no `EXTENDS ^"Virtue"`, so records.js has five of the book's six. A corpus defect; not fixed here (data/ and the corpus are off limits) — a separate task was offered. |
| 26 | Treasure kept as Foundry had it (30 for Lily, Marigold and Perry). **Owner 2026-09-25: it is stored at home.** The sheet's `Treasure` is the hero's record and stays 30; the VTT counts all of it as carried Load (upstream `system/tor2e/sheet.js` `loadTotal`), so the three draw **Weary** until upstream separates carried from stored | The book's *Carrying Treasure* counts only what is carried. Zeroing the field would lose the record; the fix belongs upstream (a task was offered), not in this instance's data. |
| 27 | Portraits: the four heroes' own (Már has none); Celenneth, Linnea, Eryndil, Findemir from the art folder. Not used: Hallas's portrait (a grown man; at the table Hallas is a small child), Damrod, Eira, Garin (not in this campaign), `character.png` and the 14 May image (unidentified), the Timeline photos and David Day map (O3) | Only faces the table has met, drawn as they were met. |
...
(drop them in `campaign/source/foundry/` and run the build). Upstream: stored Treasure is counted as Load (decision 26). The corpus's *Hardiness*
(decision 25), being fixed in another session.
