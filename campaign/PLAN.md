...
| 26 | Treasure kept as Foundry had it (30 for Lily, Marigold and Perry), though the sheet then counts it as carried Load and draws them **Weary** (Lily: Load 42 against Endurance 23) | It is a campaign fact whether that treasure is carried; flagged to the owner rather than changed. |
...
(drop them in `campaign/source/foundry/` and run the build). Decision 26 (carried Treasure). The
corpus's *Hardiness* (decision 25), outside this repo.
