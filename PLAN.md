...
| 14 | **Two corpus gaps the creator works around without inventing anything:** each culture's two Combat Proficiency ranks (the book prints *Bows OR Swords 2 · Choose one Combat Proficiency 1*; the corpus keeps both lines but not the numbers, since the extraction's table read only the labels) and which two Skills the book underlines as the Favoured choice. The creator shows both printed instructions verbatim, lets the player enter the two ranks and pick the Favoured Skill, and says why beside the control. Reported to the corpus | Ground rules: never patch the corpus in the tool, never state a number the corpus does not carry. The fix is a re-extraction of the core (the PDF page has the numbers). |
| 15 | **The six Calling features** (Leadership, Shadow-lore …) are not among the corpus's 24 typed Distinctive Features; the Calling carries each as a string, so the sheet holds it as a name without a hash | Reported to the corpus. |
