...
Exit 0 = both clean. Never weaken this to make a build pass: fix the build.
    ...
    python3 build/verify_data.py [<path to titterpig-dsl-tor2e/0.5>]
"""
import json
import os
import re
...
from build_data import BOOKS, ACTOR_DIRS, DEFAULT_CORPUS, KINDS, LORE_EXTS, corpus_files  # noqa: E402
...
BUILD_KEYS = {
    ...
    "container": "the file's container word (BASE / EXTENSION / ARC / FRAME)",
    "system": "the system id from engine/config.js",
    "form": "the entity's declaration form (DEF / ACTOR)",
    "main": "the data file paths of a book",
    "cid": "the file's container id (skipped as a header value)",
    "dir": "the actor directory a record's file sits in",
    "scenes": "a Part's SCENE_REF ids",
    "kw": "the DSL keyword a block is written with (SCENE / LOCATION / ENTRY / PHASE)",
    "t": "what a step of an adventure's document order is (prop / location / scene / entity / entry)",
}
BLOB = re.compile(r"var d=(\{.*?\});var T=window\.TOR2E", re.S)
...
def main():
    ...
    # text this build writes of its own: the book labels, the actor-chapter labels, the kinds,
    # the data file paths, and the DSL's own type words as field labels.
    ours = set(TYPE_WORDS) | KINDS | {b["label"] for b in BOOKS} | {label for _d, label in ACTOR_DIRS}
    ours |= {d + "/" for d, _label in ACTOR_DIRS}
